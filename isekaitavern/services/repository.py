...
class RedisClient:
    ...
    async def lpush(self, key: str, *value: FieldT, expired_time: int | None = None) -> None:
        if expired_time:
            self.expire_times[key] = expired_time
        await ensure_awaitable(self.redis.lpush(key, *value))
        await self.redis.expire(key, self.expire_times[key])

    async def rpush(self, key: str, *value: FieldT, expired_time: int | None = None) -> None:
        if expired_time:
            self.expire_times[key] = expired_time
        await ensure_awaitable(self.redis.rpush(key, *value))
        await self.redis.expire(key, self.expire_times[key])

    async def lpop(self, key: str) -> str:
        value = await ensure_awaitable(self.redis.lpop(key))
        assert isinstance(value, str), f"Expected str, got {type(value), value}"
        await self.redis.expire(key, self.expire_times[key])
        return value

    async def rpop(self, key: str) -> str:
        value = await ensure_awaitable(self.redis.rpop(key))
        assert isinstance(value, str), f"Expected str, got {type(value), value}"
        await self.redis.expire(key, self.expire_times[key])
        return value

    async def lrange(self, key: str, start: int, end: int) -> list[str]:
        value = await ensure_awaitable(self.redis.lrange(key, start, end))
        await self.redis.expire(key, self.expire_times[key])
        return value

    async def llen(self, key: str) -> int:
        value = await ensure_awaitable(self.redis.llen(key))
        await self.redis.expire(key, self.expire_times[key])
        return value
