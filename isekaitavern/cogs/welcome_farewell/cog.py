...
class WelcomeFarewellCog(commands.Cog, name="guild_settings"):
    ...
    async def welcome_enable(self, interaction: discord.Interaction):
        ...
        await self.repo.set_model(WelcomeModel, interaction.guild.id, enabled=True)
    ...
    async def farewell_enable(self, interaction: discord.Interaction):
        assert interaction.guild is not None
        await self.repo.set_model(FarewellModel, interaction.guild.id, enabled=True)
    ...
    async def welcome_disable(self, interaction: discord.Interaction):
        assert interaction.guild is not None
        await self.repo.set_model(WelcomeModel, interaction.guild.id, enabled=False)
    ...
    async def farewell_disable(self, interaction: discord.Interaction):
        assert interaction.guild is not None
        await self.repo.set_model(FarewellModel, interaction.guild.id, enabled=False)
    ...
    async def welcome_message(self, interaction: discord.Interaction, message: str):
        ...
        await self.repo.set_model(WelcomeModel, interaction.guild.id, description=message)
        model = await self.repo.get_model(WelcomeModel, interaction.guild.id)
        assert model is not None
        embed = self.service.build_embed(model, interaction.user)
    ...
    async def farewell_message(self, interaction: discord.Interaction, message: str):
        assert interaction.guild is not None
        assert isinstance(interaction.user, discord.Member)
        await self.repo.set_model(FarewellModel, interaction.guild.id, description=message)
        model = await self.repo.get_model(FarewellModel, interaction.guild.id)
        assert model is not None
        embed = self.service.build_embed(model, interaction.user)
        await interaction.response.send_message(embed=embed)
    ...
    async def welcome_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        assert interaction.guild is not None
        await self.repo.set_model(WelcomeModel, interaction.guild.id, channel_id=channel.id)
    ...
    async def farewell_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        assert interaction.guild is not None
        await self.repo.set_model(FarewellModel, interaction.guild.id, channel_id=channel.id)
    ...
    async def welcome_title(self, interaction: discord.Interaction, title: str):
        assert interaction.guild is not None
        assert isinstance(interaction.user, discord.Member)
        await self.repo.set_model(WelcomeModel, interaction.guild.id, title=title)
        model = await self.repo.get_model(WelcomeModel, interaction.guild.id)
        assert model is not None
        embed = self.service.build_embed(model, interaction.user)
        await interaction.response.send_message(embed=embed)
    ...
    async def farewell_title(self, interaction: discord.Interaction, title: str):
        assert interaction.guild is not None
        assert isinstance(interaction.user, discord.Member)
        await self.repo.set_model(FarewellModel, interaction.guild.id, title=title)
        model = await self.repo.get_model(FarewellModel, interaction.guild.id)
        assert model is not None
        embed = self.service.build_embed(model, interaction.user)
        await interaction.response.send_message(embed=embed)
    ...
    async def welcome_color(self, interaction: discord.Interaction, color: str) -> None:
        ...
        if color_value:
            await self.repo.set_model(WelcomeModel, interaction.guild.id, color=color_value)
            model = await self.repo.get_model(WelcomeModel, interaction.guild.id)
            assert model is not None
            embed = self.service.build_embed(model, interaction.user)
            await interaction.response.send_message(embed=embed)
    ...
    async def farewell_color(self, interaction: discord.Interaction, color: str) -> None:
        ...
        color_value = self.service.get_color(color)
        if color_value:
            await self.repo.set_model(FarewellModel, interaction.guild.id, color=color_value)
            model = await self.repo.get_model(FarewellModel, interaction.guild.id)
            assert model is not None
            embed = self.service.build_embed(model, interaction.user)
            await interaction.response.send_message(embed=embed)
    ...
    async def welcome_thumbnail(self, interaction: discord.Interaction, url: str = "") -> None:
        assert interaction.guild is not None
        assert isinstance(interaction.user, discord.Member)
        await self.repo.set_model(WelcomeModel, interaction.guild.id, thumbnail_url=url)
        model = await self.repo.get_model(WelcomeModel, interaction.guild.id)
        assert model is not None
        embed = self.service.build_embed(model, interaction.user)
        await interaction.response.send_message(embed=embed)
    ...
    async def farewell_thumbnail(self, interaction: discord.Interaction, url: str = "") -> None:
        assert interaction.guild is not None
        assert isinstance(interaction.user, discord.Member)
        await self.repo.set_model(FarewellModel, interaction.guild.id, thumbnail_url=url)
        model = await self.repo.get_model(FarewellModel, interaction.guild.id)
        assert model is not None
        embed = self.service.build_embed(model, interaction.user)
        await interaction.response.send_message(embed=embed)
    ...
    async def welcome_image(self, interaction: discord.Interaction, url: str = "") -> None:
        assert interaction.guild is not None
        assert isinstance(interaction.user, discord.Member)
        await self.repo.set_model(WelcomeModel, interaction.guild.id, image_url=url)
        model = await self.repo.get_model(WelcomeModel, interaction.guild.id)
        assert model is not None
        embed = self.service.build_embed(model, interaction.user)
        await interaction.response.send_message(embed=embed)
    ...
    async def farewell_image(self, interaction: discord.Interaction, url: str = "") -> None:
        assert interaction.guild is not None
        assert isinstance(interaction.user, discord.Member)
        await self.repo.set_model(FarewellModel, interaction.guild.id, image_url=url)
        model = await self.repo.get_model(FarewellModel, interaction.guild.id)
        assert model is not None
        embed = self.service.build_embed(model, interaction.user)
        await interaction.response.send_message(embed=embed)
    ...
    async def welcome_demo(self, interaction: discord.Interaction) -> None:
        ...
        model = await self.repo.get_model(WelcomeModel, interaction.guild.id)
        if not model:
            await self.repo.set_model(WelcomeModel, interaction.guild.id)
            model = await self.repo.get_model(WelcomeModel, interaction.guild.id)
        assert model is not None
        embed = self.service.build_embed(model, interaction.user)
        await interaction.response.send_message(embed=embed)
    ...
    async def farewell_demo(self, interaction: discord.Interaction) -> None:
        ...
        model = await self.repo.get_model(FarewellModel, interaction.guild.id)
        if not model:
            await self.repo.set_model(FarewellModel, interaction.guild.id)
            model = await self.repo.get_model(FarewellModel, interaction.guild.id)
        assert model is not None
        embed = self.service.build_embed(model, interaction.user)
        await interaction.response.send_message(embed=embed)
